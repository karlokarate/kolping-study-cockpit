...
(function(){
  ...
  function genId() {
    if (window.crypto && window.crypto.getRandomValues) {
      ...
      var randomPart = Array.from(array, function(num) {
        return num.toString(16);
      }).join('');
      return Date.now() + '_' + randomPart;
    } else {
      // Fallback for environments without Web Crypto API (non-cryptographic)
  ...
  function post(data) {
    try { 
      RecorderBridge.postMessage(JSON.stringify(data)); 
    } catch(e) {
      console.warn('RecorderBridge post failed:', e);
    }
  }
  ...
  window.fetch = async function(input, init) {
    ...
    try {
      ...
      }).catch(function(e){ console.warn('Failed to read response body:', e); });
      return response;
    } catch(e) { 
