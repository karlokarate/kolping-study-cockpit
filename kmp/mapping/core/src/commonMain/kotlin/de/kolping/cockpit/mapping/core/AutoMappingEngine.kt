...
class AutoMappingEngine {
    private val hubPatterns = listOf(
        ...
        Regex("^/myStudent(/|\$)", RegexOption.IGNORE_CASE),
        Regex("^/dashboard(/|\$)", RegexOption.IGNORE_CASE),
        Regex("^/course/view\\.php", RegexOption.IGNORE_CASE)
    )
    private val learnedHubSignatures = mutableSetOf<String>()

    fun detectHub(url: String, signature: String): Boolean {
        // Extract path portion of the URL (without scheme, host, or query)
        val withoutScheme = url.substringAfter("://", url)
        val afterHost = withoutScheme.substringAfter("/", "")
        val rawPath = afterHost.substringBefore("?")
        val path = "/" + rawPath.trimStart('/')
        
        if (hubPatterns.any { it.containsMatchIn(path) }) return true
                ...
                is Event.Navigation -> {
                    if (event.phase == Event.Phase.FINISHED) {
                        ...
                        if (existingNode == null) {
                            val node = ChainPoint(
                                id = nodeId,
                                name = extractLastPathSegment(event.url),
                                url = event.url,
                                signature = sig,
                                isHub = isHub
                            )
